...
try{
    var xml = fs.readFileSync(ifileUrl,'utf8');
} catch (err) {
    console.log(err);
    process.exit();
}

//Parse xml file -> javascript object
parse(xml,function(err,parsedTree) {

    // console.log(parsedTree.recGeneTree.phylogeny[0].clade[0]);
    var root = d3.hierarchy(parsedTree.recGeneTree.phylogeny[0].clade[0],function(d){return d.clade});
    ...
    .attr('stroke-dasharray',function(d){
        var le = d.data.eventsRec.slice(-1)[0];
        if(le.transferBack || le.bifurcationOut)
            return '5,5';
    });
